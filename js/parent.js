...
Drupal.jqui_dialog.open = function(c_options) {
  ...
  if(self.options.width != undefined) {
    self.container.dialog('option', {width: self.options.width+26});
    self.iframe.width(self.options.width);
  }
  
  if(self.options.height != undefined) {
    self.container.dialog('option', {width: self.options.width+12});
...
Drupal.jqui_dialog.resize = function() {    
  ...
  var animationOptions = $.extend({width: frameSize.width+paddingWidth, height: dialogSize.height}, dialogPosition);
  self.iframe.hide();
  self.container.dialog('option', {width: frameSize.width+paddingWidth, height: dialogSize.height});  
